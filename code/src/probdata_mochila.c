...
SCIP_RETCODE probdataCreate(
   ...
   parametersT*          param               /**< pointer to the config parameters */
   )
{
   assert(scip != NULL);
...
SCIP_DECL_PROBTRANS(probtransMochila)
{
   ...
   SCIP_CALL( probdataCreate(scip, targetdata, sourcedata->probname, sourcedata->vars, sourcedata->conss, sourcedata->nvars, sourcedata->ncons, sourcedata->I, sourcedata->param) );
   ...
   )
{
   ...
   for( j = 0; j < I->nS; nvars++, ++j )
   {
      ...
      if(!relaxed){
         ...
         SCIP_CALL( SCIPcreateVarBasic(scip, &var, name, 0.0, (double) I->S[j].n - I->S[j].h, (double) -I->S[j].d, SCIP_VARTYPE_CONTINUOUS) );
      }
      assert(var != NULL);
      /* save the pointer to the created var */
      vars[nvars]=var;
   ...
   /* create problem data */
   SCIP_CALL( probdataCreate(scip, &probdata, probname, vars, conss, nvars, ncons, I, param) );
...
const parametersT* SCIPprobdataGetParam(
   SCIP_PROBDATA*        probdata            /**< problem data */
   )
{
   return probdata->param;
}
/**@} */
